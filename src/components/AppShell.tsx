...
import { ScanLine, Users, LayoutDashboard, LogOut, ShieldCheck, Receipt, GraduationCap, Home, FileSpreadsheet } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground font-sans selection:bg-blue-500/20">
      <aside className="w-80 border-r border-sidebar-border flex flex-col z-20 bg-sidebar text-sidebar-foreground">
        ...
        <nav className="flex-1 px-4 space-y-1.5">
          ...
          <NavItem to="/" icon={<Home className="h-5 w-5" />} label="Accueil" />
          ...
          <NavItem to="/import-excel" icon={<Receipt className="h-5 w-5" />} label="Import Banque" />
          <NavItem to="/students" icon={<Users className="h-5 w-5" />} label="Base Étudiants" />
          <NavItem to="/import-export" icon={<FileSpreadsheet className="h-5 w-5" />} label="Import/Export Excel" />
          <NavItem to="/classes" icon={<GraduationCap className="h-5 w-5" />} label="Gestion Classes" />
      ...
      <main className="flex-1 relative overflow-hidden">
        ...
        <div className="h-full overflow-auto">
          <div className="max-w-7xl mx-auto p-8 lg:p-14 animate-in fade-in duration-700">
            {children}
          </div>
        </div>
      </main>
